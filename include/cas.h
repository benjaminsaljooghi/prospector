...
struct Prediction : public Locus {
    ...
    string to_string_debug() {
        return to_string_summary();
    }

    string to_string_summary() {
        std::string result = fmt::format("{}\t{}\t{}\t{}", start, end, id, score);
        return result;
    }
...
struct Fragment {
    ...
    bool repetition_problem() {
        auto kmers = Util::kmerize(this->protein_sequence,
                                   CasProfileUtil::k); // no need to do this sicnce it's already been kmerized etc but keep it for now
        const int domain = 15;
    ...
    string to_string_debug() {
        string amino_domain = Util::translate_genome(*reference_genome, genome_begin, genome_final,
                                                     reference_translation->pos);
        string amino_gene = Util::translate_genome(*reference_genome, expanded_genome_begin, expanded_genome_final,
                                                   reference_translation->pos);

        string dna_domain = reference_genome->substr(genome_begin, genome_final - genome_begin);
        string dna_gene = reference_genome->substr(expanded_genome_begin,
                                                   expanded_genome_final - expanded_genome_begin);
        ...
        out << fmt::format("{}\t{}\t{}\n", reference_translation->pos ? "+" : "-", reference_profile->identifier,
                           CasProfileUtil::domain_table_fetch(reference_profile->identifier));
...
namespace Cas {
    ...
    vector<Prediction *> predict_cas(vector<CasProfile *> &profiles, string &genome, ull offset);
}

struct FrequencyMap {
    phmap::flat_hash_map<kmer, double> freq_map;
    string file_path = Config::path_output / "frequency_map";
